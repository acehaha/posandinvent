...
namespace pharmacymanagementandpos
{
    public partial class DailySale : Form
    {
        ...
        public void LoadSold()
            ...
            {
                cm = new SqlCommand("select c.id, transno, c.pcode, p.pdesc, p.price, c.qty, c.disc, c.total from tbCart as c inner join tbProduct as p on c.pcode = p.pcode where status like 'Sold' and sdate between '" + dtFrom.Value + "' and '" + dtTo.Value + "'", cn);
            }
            else
            {
                cm = new SqlCommand("select c.id, transno, c.pcode, p.pdesc, p.price, c.qty, c.disc, c.total from tbCart as c inner join tbProduct as p on c.pcode = p.pcode where status like 'Sold'  and sdate between '" + dtFrom.Value + "' and '" + dtTo.Value + "' and cashier like '" + cboCashier.Text + "'", cn);
            }
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void dgvSold_CellContentClick_1(object sender, DataGridViewCellEventArgs e)
        {
           ...
           // {
             ...
             //   cancelOrder.txtDisc.Text = dgvSold.Rows[e.RowIndex].Cells[7].Value.ToString();
              ...
              //  {
                   // cancelOrder.txtCancelledBy.Text = main.labelUsername.Text;
               // }
              //  else
              //  {
        ...
        private void btnPrintTopSell_Click(object sender, EventArgs e)
        {
            ...
            string param = "Date From: " + dtFrom.Value.ToShortDateString() + "' and '" + dtTo.Value.ToShortDateString();
            if (cboCashier.Text == "All Cashier")
            {
                report.LoadDailyReport("select c.id, transno, c.pcode, p.pdesc, p.price, c.qty, c.disc as discount, c.total from tbCart as c inner join tbProduct as p on c.pcode = p.pcode where status like 'Sold' and sdate between '" + dtFrom.Value + "' and '" + dtTo.Value + "'",param, cboCashier.Text);
            }
            else
            {
                report.LoadDailyReport("select c.id, transno, c.pcode, p.pdesc, p.price, c.qty, c.disc as discount, c.total from tbCart as c inner join tbProduct as p on c.pcode = p.pcode where status like 'Sold'  and sdate between '" + dtFrom.Value + "' and '" + dtTo.Value + "' and cashier like '" + cboCashier.Text + "'", param, cboCashier.Text);
            }
            report.ShowDialog();
        }
    }
